...
pub fn os_instance_init(instance: &mut Mize) -> MizeResult<()> {
    ...
    match std::env::var("MIZE_CONFIG_FILE") {
        Ok(config_file_path) => {
            ...
            trace!(
                "config after MIZE_CONFIG_FILE env var: {}",
    ...
    match std::env::var("MIZE_CONFIG") {
        Ok(config_string) => {
            ...
            trace!(
                "config after MIZE_CONFIG env var: {}",
                instance.get("self/config")?
            );
        }
        Err(var_err) => match var_err {
    ...
    let mut store_path = match instance.get("self/config/store_path")?.value_string() {
        ...
        Err(e) => {
            ...
            let home_dir = if let Some(dir) = std::env::home_dir() {
                ...
                return Err(mize_err!(
                    "could not get home_dir and store_path is not set in the config"
                ));
            };
            ...
            instance.set_blocking(
                ...
                store_path.clone().into_item_data(),
            )?;
...
pub fn get_module_hash(
    instance: &mut Mize,
    name: &str,
    mut options: ItemData,
) -> MizeResult<(String, String)> {
    ...
    if name_parts
        .next()
        .ok_or(mize_err!("no 0th element in the modName"))?
        == "cross"
    {
        let system = name_parts
            .next()
            .ok_or(mize_err!("no 1th element in the modName"))?;
...
fn download_module(
    ...
    let tmp_gz_file = if !PathBuf::from(tmp_file_path_gz.clone()).exists() {
        ...
        debug!(
            "downloading module '{}' with hash: {}",
            module_name, module_hash
        );

        let response = http_req::request::get(
            format!(
                "http://{}/mize/dist/{}-{}.tar.gz",
                module_url, module_hash, module_name
            ),
            &mut tmp_gz_file,
        )?;
        ...
        if code != 200.into() {
            return Err(mize_err!(
                "failed to get module '{}' with hash '{}' and selector: {}",
                module_name,
                module_hash,
                module_selector
            )
            .msg(format!(
                "the http request to fetch the module returned a status code of: {}",
                code
            )));
        }
...
pub fn fetch_module(instance: &mut Mize, module_name: &str) -> MizeResult<String> {
    let module_name_with_slashes = module_name.replace(".", "/");
    if let Ok(module_path) = instance
        .get(format!(
        ...
        .value_string()
    {
        ...
        let (module_hash, module_selector) =
            get_module_hash(instance, module_name, ItemData::new())?;
...
pub fn load_module(instance: &mut Mize, module_name: &str, path: Option<String>) -> MizeResult<()> {
    let module_path = if path.is_some() {
        format!("{}/lib/libmize_module_{}.so", path.unwrap(), module_name)
    } else {
        format!(
            "{}/lib/libmize_module_{}.so",
            fetch_module(instance, module_name)?,
            module_name
        )
    };
    ...
    let func: libloading::Symbol<
        unsafe extern "C" fn(&mut Box<dyn Module + Send + Sync>, Mize) -> (),
    > = unsafe { lib.get(format!("get_mize_module_{}", module_name).as_bytes())? };
