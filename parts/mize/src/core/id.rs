use core::fmt;
use interner::{
    shared::{SharedPool, SharedString, SharedVecString},
    Pooled,
};
...
pub trait IntoMizeId {
    fn to_mize_id(self, instance: &Mize) -> MizeResult<MizeId>;
}
...
impl MizeId {
    pub fn store_part(&self) -> &str {
        self.path
            .iter()
            .nth(0)
            .expect("an empty MizeId found, that should absolutely not be possible!!!!")
            .as_str()
    }
...
mod test {
    ...
    fn test_id() -> MizeResult<()> {
        ...
        assert_eq!(
            vec!["config".to_owned(), "hi".to_owned()],
            id_one.after_store_part()
        );

        let id_two = instance.new_id("test.ns:hello/config/hi")?;
        assert_eq!(
            instance.namespace_from_string("test.ns".to_owned())?,
            id_two.namespace()
        );
